...
function playerAttack() {
  ...
  if (success) {
    ...
    const enemyHealthDisplay = document.getElementById('enemy-combat-health');
    if (enemyHealthDisplay) {
      enemyHealthDisplay.textContent = enemy.health;
    }
    
    const enemyMaxHealthDisplay = document.getElementById('enemy-combat-max-health');
    if (enemyMaxHealthDisplay) {
      enemyMaxHealthDisplay.textContent = enemy.maxHealth;
    }
    
    const enemyHealthFillDisplay = document.getElementById('enemy-health-fill');
    if (enemyHealthFillDisplay) {
      enemyHealthFillDisplay.style.width = `${(enemy.health / enemy.maxHealth) * 100}%`;
    }
